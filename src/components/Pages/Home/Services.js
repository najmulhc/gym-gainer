...
const Services = () => {
  ...
  return (
    <div className="my-8 container mx-auto" id="service">
      <h1 className="text-5xl text-center font-bold text-komola mb-4 ">
        Services I provide
      </h1>
        ...
        {services.map((service) => {
          return (
            <div className="p-8 shadow-md bg-orange-200 bg-opacity-25 flex flex-col justify-between">
                <img src={service.img} className="mb-6" alt="" />
              <h1 className=" text-grey text-3xl font-bold ">{service.name}</h1>
              <p className="text-md mt-4  text-normal font-medium">
                {service.description}
              </p>
              <button className="text-white p-2 bg-komola w-full mt-12 text-2xl ">
                <Link to="/checkout" state={{ ser: service }}>
                  Check this out
                </Link>
              </button>
            </div>
